#!/bin/ash
  ...
  if [ $? == 1 ]; then
    ...
    if echo ${END} | grep -q 'will expire'; then
      elevenLogJSON info "script \"${SCRIPT}\" certificate will expire in the next three days, replacing ..."
      echo '{}' | jq \
      ...
      for IP in ${HORIZON_VIEW_UAG_NODES}; do
        ...
        if [ ! $? == 0 ]; then
          ...
          elevenLogJSON info "script \"${SCRIPT}\" certificate upadted on Unified Access Gateway [${IP}]"
        fi
      done
    else
      elevenLogJSON info "script \"${SCRIPT}\" no updated of certificate needed, does not expire in the next three days"
    fi
  else
